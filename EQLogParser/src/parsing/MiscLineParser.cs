...
namespace EQLogParser
{
    class MiscLineParser
    {
        ...
        private static readonly Regex ExperienceRegex =
            new Regex(
      ...
      { "sliced", 1 }, { "stabbed", 1 }, { "surrounded", 1 }, { "struck", 1 }, { "stunned", 1 }, { "targeted", 1 }, { "withered", 1 }
        ...
        public static void Process(LineData lineData)
        {
            ...
            try
            {
                ...
                if (!handled)
                {
                    Match levelMatch = LevelRegex.Match(lineData.Action);

                    if (levelMatch.Success &&
                        int.TryParse(levelMatch.Groups["level"].Value, out int level))
                    {
                        DataManager.Instance.AddLevelRecord(
                          new LevelRecord
                          {
                              Level = level
                          },
                ...
                if (!handled)
                {
                    Match zoneMatch = ZoneRegex.Match(lineData.Action);

                    if (zoneMatch.Success)
                    {
                        ZoneRecord record =
                          ParseZoneRecord(zoneMatch.Groups["zoneText"].Value);

                        DataManager.Instance.AddZoneRecord(
                          record,
                          lineData.BeginTime);
        ...
        private static ZoneRecord ParseZoneRecord(string zoneText)
        {
            ...
            string remaining = zoneText.Trim();

            // Extract the final parenthetical name:
            // "Befallen 4 (Refined)"
            // "The Plane of Hate - Group 3 (Fused)"
            int openParen = remaining.LastIndexOf(" (", StringComparison.Ordinal);

            if (openParen >= 0 &&
                remaining.EndsWith(")", StringComparison.Ordinal))
            {
                record.DifficultyName = remaining
                  .Substring(
                    openParen + 2,
                    remaining.Length - openParen - 3)
                  .Trim();
            ...
            // Extract the final numeric difficulty.
            int finalSpace = remaining.LastIndexOf(' ');

            if (finalSpace >= 0)
            {
                string difficultyText =
                  remaining.Substring(finalSpace + 1);
                  ...
                  out int difficulty))
                {
                    ...
                    remaining = remaining
                      .Substring(0, finalSpace)
                      .Trim();
                }
            }

            // Extract an instance type such as:
            // "The Plane of Hate - Group"
            ...
            {
                record.InstanceType = "Group";

                remaining = remaining
                  .Substring(0, remaining.Length - groupSuffix.Length)
                  .Trim();
            }

            record.Zone = remaining;
